...
class Rectangle(Dot):
    ...
    def draw_figure(self):
        for i in range(self.x1, self.x2 + 1):
            super().draw_figure(i, self.y1)
            super().draw_figure(i, self.y2)
        for i in range(self.y1, self.y2 + 1):
            super().draw_figure(self.x1, i)
            super().draw_figure(self.x2, i)

    def clear_figure(self):
        for i in range(self.x1, self.x2 + 1):
            super().clear_figure(i, self.y1)
            super().clear_figure(i, self.y2)
        for i in range(self.y1, self.y2 + 1):
            super().clear_figure(self.x1, i)
            super().clear_figure(self.x2, i)
